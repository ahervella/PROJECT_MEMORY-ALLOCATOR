...
- **A portable, single-header integration (`CustomMemoryAllocator.h`).** Global placement `operator new`/`operator new[]` overloads taking an `IMemoryAllocator&`, so usage looks like ordinary C++: `new (allocator) T(args...)`.
